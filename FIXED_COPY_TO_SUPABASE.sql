-- =============================================================================
-- TALENTPATRIOT USER-ORGANIZATION ASSIGNMENT SETUP (FIXED VERSION)
-- Copy and paste this ENTIRE script into Supabase SQL Editor and run it
-- This handles existing table structures and missing columns
-- =============================================================================

-- Enable required extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Create enums if they don't exist (they should already exist from cleanup script)
DO $$ BEGIN
...
-- Handle user_profiles table - check if it exists and add missing columns
DO $$
BEGIN
    ...
    ELSE
        RAISE NOTICE 'ℹ user_profiles table already exists, checking columns...';
        ...
        IF NOT EXISTS (
            ...
            ALTER TABLE public.user_profiles ADD COLUMN role user_role DEFAULT 'hiring_manager' NOT NULL;
        ...
        IF NOT EXISTS (
            ...
            ALTER TABLE public.user_profiles ADD COLUMN created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL;
        ...
        IF NOT EXISTS (
            ...
            ALTER TABLE public.user_profiles ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL;
        ...
        IF NOT EXISTS (
            ...
            ALTER TABLE public.user_profiles ADD COLUMN metadata JSONB DEFAULT '{}'::jsonb;
            RAISE NOTICE '✓ Added metadata column to user_profiles';
        END IF;
    END IF;
END $$;
...
-- Fix existing users without complete profiles (only if role column now exists)
DO $$
BEGIN
    -- Only try to insert/update if the role column exists
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'user_profiles' AND column_name = 'role'
    ) THEN
        ...
        -- Update existing profiles that don't have role set
        UPDATE public.user_profiles 
...
EXCEPTION
    WHEN others THEN
        RAISE NOTICE '⚠ Could not fix existing user profiles: %', SQLERRM;
END $$;
