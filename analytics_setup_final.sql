...
-- Skills Analytics View (FIXED: Handles text-based proficiency levels)
...
SELECT 
    ...
    -- FIXED: Map text proficiency levels to numbers, or return the text value
    AVG(CASE 
        WHEN c.skill_levels IS NOT NULL AND c.skill_levels ? skill_item THEN
            CASE (c.skill_levels ->> skill_item)
                ...
                ELSE 
                    ...
                    CASE 
                        WHEN (c.skill_levels ->> skill_item) ~ '^[0-9]+$' 
    ...
    -- Most common proficiency level for this skill (as text)
    MODE() WITHIN GROUP (ORDER BY 
        CASE 
            WHEN c.skill_levels IS NOT NULL AND c.skill_levels ? skill_item 
            THEN c.skill_levels ->> skill_item
            ELSE NULL 
        END
    ) as most_common_proficiency
...
SELECT 
    'TalentPatriot Analytics Setup Complete (FINAL)' as status,
...
SELECT 'Final analytics infrastructure ready - handles text proficiency levels properly' as result;