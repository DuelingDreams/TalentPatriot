...
import { MobilePipeline } from '@/features/jobs/components/MobilePipeline'
...
import { ResumeUpload } from '@/features/candidates/components/resume/ResumeUpload'
...
// Fallback ApplicationCard removed - using EnhancedApplicationCard exclusively
...
export default function JobPipeline() {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      ...
      try {
        // Use applicationId directly from the drag event - this is the job_candidate.id
        const applicationId = application.id
        
        if (!applicationId) {
          throw new Error('Cannot move application: applicationId not found')
        }
        
        const success = await retryMove(applicationId, newColumn.id)
        if (success) {
          toast({
            title: "Success",
