-- Database Consistency Update - August 2025 (Fixed Version)
...
-- 8. Create schema versioning table FIRST, then insert record
CREATE TABLE IF NOT EXISTS schema_migrations (
    version VARCHAR(255) PRIMARY KEY,
    applied_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Now safely insert the migration record
INSERT INTO schema_migrations (version) 
VALUES ('database_consistency_update_august_2025_fixed')
ON CONFLICT (version) DO UPDATE SET applied_at = NOW();

-- 9. Verify data integrity (these are SELECT statements for information)
...
SELECT 'Database consistency update completed successfully!' as status;