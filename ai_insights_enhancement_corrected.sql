-- AI Insights Enhancement SQL Script (CORRECTED FOR ORGANIZATION_ROLE ENUM)
-- Optional tables to enhance AI insights with persistent storage and history
...
CREATE POLICY "Users can insert AI insights cache for their organization" ON ai_insights_cache
  FOR INSERT WITH CHECK (
    org_id IN (
      SELECT uo.org_id FROM user_organizations uo 
      WHERE uo.user_id = auth.uid()
      AND uo.role IN ('admin', 'recruiter')
    )
  );
...
-- SELECT cron.schedule('cleanup-ai-insights-cache', '0 */6 * * *', 'SELECT cleanup_expired_ai_insights_cache();');
