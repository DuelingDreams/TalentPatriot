-- ===============================================
-- TalentPatriot ATS - Analytics & Reports Setup (UPDATED)
-- ===============================================
-- This script creates materialized views, indexes, and helper tables
-- for comprehensive analytics and reporting functionality.
-- 
-- UPDATED FOR: New schema fields, skill_levels JSON, and proper table references
-- 
-- INSTRUCTIONS:
...
-- ===============================================
-- 4. PERFORMANCE INDEXES (Updated for new schema)
-- ===============================================
...
-- Specialized indexes for skills and metadata analytics (Updated)
...
-- NEW: Index for skill_levels JSON column
...
-- Additional indexes for application metadata
...
-- ===============================================
-- 5. MATERIALIZED VIEWS FOR CORE METRICS (Updated)
-- ===============================================

-- Pipeline Metrics View (Updated for current schema)
...
SELECT 
    ...
    COUNT(*) FILTER (WHERE jc.stage = 'phone_screen') as screening_count,
    ...
    CASE 
        ...
        THEN (COUNT(*) FILTER (WHERE jc.stage = 'phone_screen')::float / COUNT(*) FILTER (WHERE jc.stage = 'applied')) * 100
        ELSE 0 
    END as applied_to_screening_rate,
    
    CASE 
        WHEN COUNT(*) FILTER (WHERE jc.stage = 'phone_screen') > 0 
        THEN (COUNT(*) FILTER (WHERE jc.stage = 'interview')::float / COUNT(*) FILTER (WHERE jc.stage = 'phone_screen')) * 100
        ELSE 0 
...
-- Candidate Source Analytics View (Updated)
...
-- Time to Hire Analytics View (Updated)
...
-- Skills Analytics View (Updated with skill_levels support)
...
SELECT 
    ...
    -- NEW: Average proficiency level for this skill
    AVG(CASE 
...
-- Recruiter Performance View (Updated to use user_profiles)
...
LEFT JOIN user_profiles up ON j.assigned_to = up.id  -- FIXED: Use user_profiles instead of users
WHERE j.assigned_to IS NOT NULL
GROUP BY j.org_id, j.assigned_to, up.first_name, up.last_name;

-- Client Performance View (Unchanged - already correct)
...
SELECT 
    ...
    COUNT(DISTINCT j.id) FILTER (WHERE j.status = 'open' AND j.created_at < CURRENT_DATE - INTERVAL '30 days') as aging_jobs_30_days,
    COUNT(DISTINCT j.id) FILTER (WHERE j.status = 'open' AND j.created_at < CURRENT_DATE - INTERVAL '60 days') as aging_jobs_60_days
...
-- Function to get comprehensive org analytics (Updated)
...
BEGIN
    ...
    -- Add top skills (Updated with proficiency levels)
    result := result || jsonb_build_object(
        'top_skills', (
            SELECT jsonb_agg(
                jsonb_build_object(
                    ...
                    'avg_proficiency', avg_proficiency_level
                )
            )
...
-- Function to create daily analytics snapshot (Updated)
...
-- ===============================================
-- 7. MATERIALIZED VIEW INDEXES (Required for concurrent refresh)
-- ===============================================
...
LEFT JOIN (
    SELECT org_id, COUNT(*) as total_jobs, COUNT(*) FILTER (WHERE status = 'open') as active_jobs
...
SELECT 
    'TalentPatriot Analytics Setup Complete (Updated)' as status,
...
SELECT 'Updated analytics infrastructure ready - aligned with current schema and Reports page requirements' as result;