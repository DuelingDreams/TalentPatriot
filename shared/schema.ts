...
// OAuth Sessions (temporary sessions for OAuth flow - stored in Supabase)
export const oauthSessions = pgTable("oauth_sessions", {
  id: uuid("id").primaryKey().defaultRandom(),
  sessionTokenHash: text("session_token_hash").notNull(), // Hashed token for security
  userId: uuid("user_id").notNull(), // References auth.users(id)
  orgId: uuid("org_id").references(() => organizations.id).notNull(),
  returnTo: text("return_to").default('/settings/integrations'),
  stateNonce: text("state_nonce").notNull(), // For replay protection
  redirectHost: text("redirect_host"), // Original host for redirect validation
  expiresAt: timestamp("expires_at").notNull(),
  consumedAt: timestamp("consumed_at"), // Set when session is used (single-use)
  createdAt: timestamp("created_at").defaultNow().notNull(),
