-- Complete RLS Policies for TalentPatriot ATS
...
DROP POLICY IF EXISTS "interviews_access_policy" ON interviews;
DROP POLICY IF EXISTS "messages_access_policy" ON messages;
DROP POLICY IF EXISTS "message_recipients_access_policy" ON message_recipients;
...
CREATE POLICY "block_anonymous_access" ON interviews FOR ALL TO anon USING (false);
CREATE POLICY "block_anonymous_access" ON messages FOR ALL TO anon USING (false);
CREATE POLICY "block_anonymous_access" ON message_recipients FOR ALL TO anon USING (false);
...
-- INTERVIEWS TABLE
CREATE POLICY "interviews_access_policy" ON interviews
  FOR SELECT
  TO authenticated
  USING (
    -- Demo viewers see only demo data from demo org
...
CREATE POLICY "message_recipients_access_policy" ON message_recipients
  FOR SELECT
  TO authenticated
  USING (
    -- Demo viewers see only data from demo org
    (auth.jwt() ->> 'user_metadata' ->> 'role' = 'demo_viewer' 
     AND org_id = 'demo-org-fixed')
    OR
    -- Real users see data from their organizations
    (auth.jwt() ->> 'user_metadata' ->> 'role' != 'demo_viewer' 
     AND org_id IN (
       SELECT org_id FROM user_organizations 
       WHERE user_id = auth.uid()
