...
## Pipeline Kanban Board Fix (2025-10-01)
- **Issue**: Candidate cards were not moving between pipeline columns due to missing authentication middleware
- **Root Cause**: Move and reject endpoints lacked authentication, causing RLS policies to block updates
- **Fix Applied**:
  - Added `requireAuth` middleware with support for both development (x-user-id header) and production (Bearer token) authentication
  - Added `requireRecruiting` middleware for role-based authorization checks
  - Implemented column validation to ensure moves are authorized (UUID format, same job/org verification)
  - Enhanced error handling and logging throughout the move operation
  - Temporary workaround: Removed `stage` enum updates due to missing `candidate_stage` type in database (only updates `pipeline_column_id`)
- **Security Improvements**:
  - Development auth requires explicit `ALLOW_DEV_AUTH=true` or `NODE_ENV=development`
  - Bearer tokens always take precedence over development headers
  - Comprehensive authorization checks prevent cross-org data access
- **Known Issues**:
  - Database enum `candidate_stage` needs to be created via migration
  - Frontend needs Authorization header support for production use
  - Some database tables (e.g., `user_organizations`) may be missing in current environment